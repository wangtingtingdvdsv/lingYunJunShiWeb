...
export default {
  ...
  methods: {
    ...
    getOrderList() {
      var that = this;
      axios.get(config.url + '/seller/order/list')
      .then(res => {
        ...
        for(let i = 0; i < data.length; i++) {
            ...
            let seconds = data[i].delivery_time;
          
            let date = new Date(Number(seconds));
            console.log('date', date);
              ...
              let minu = date.getMinutes(); //获取分钟
            data[i].delivery_time = year+"-"+month+'-'+day+' '+hour+':'+minu;
